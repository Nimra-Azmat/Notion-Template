...
def main(db_id, spreadsheet_id, sheet_name):
    ...
    for page in pages:
        ...
        assignee_list.reverse()
        # assignee_list = list(set(assignee_list))
        notion_task.append(tasks)
        notion_task.reverse()

        dictionary = defaultdict(list)

        for key, value in zip(assignee_list, notion_task):
            dictionary[key].append(value)

    print("\n the dictionary is: \n ", dictionary)
    # print("List of assignee and tasks :", assignee_list, notion_task)

    # Create tasks in the Notion database
    task_list = [item for sublist in task_list for item in sublist]

    # print(flat_list)
    print("check each task \n", task_list)
    ...
    for i in range(len(userids) - 1):
        print(
            "-----------i loop for userid in teamspace------------\n",
            range(len(userids) - 1),
            i,
        )
        for ids in userids:  # notion teamspace user list
            print("-----------ids notion from teamspace-------------\n", ids)
            # test = (key in dictionary.keys()) == ids.strip()
            for key in dictionary:
                print(key)
                # key = key in dictionary.keys()
                test = key == ids
                print(
                    "check the response of first if condition ",
                    test,
                    key,
                    type(key),
                    ids,
            ...
            if key in dictionary == ids:  # id for notion database
                # print("id from assignee_list", assignee_list)
                print(
                    ".......ids from assignee list from notion database.......\n",
                    ids,
                    "\n........... key is: .........\n",
                    key,
                )
                for task in task_list:  # task from excel sheet
                    # print("-----------task from excel-------------\n", assignee_list)
                    print(
                        f"-------Notions list task from excel sheet :--------\n {task}\n"
                    )
                    if (
                        task[0] in dictionary == task
                    ):  # task from the notion database list
                        print(
                            f"----------task if condition task comparision -----------\n  {task[0]}"
                        )
                        print(
                            f"\n check the condition of if statment \n {task[0]==task[0]} where task from excel is {task} and from database is {task[0]}"
                        )
                        # print(f"Task '{task[0]}' already exists for user ID '{ids}'.")
                        continue
                    else:  # create task if does not exists
                        print("task is created", task[0], "for id ", ids)
                        # task_data = {
                        #     "parent": {"database_id": db_id},
                        #     "properties": {
                        #         "Tasks": {
                        #             "title": [
                        #                 {"type": "text", "text": {"content": task[0]}}
                        #             ]
                        #         },
                        #         "Status": {
                        #             "select": {"name": "Incomplete", "color": "red"}
                        #         },
                        ...
                        #     "POST", createUrl, headers=headers, data=data
                        # )
                        # print(
